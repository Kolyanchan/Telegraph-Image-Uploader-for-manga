...
dr = input("Введите дерикторию (Используйте'/' вместо '\\'): ")     # Указание дириктории страниц манги / Specifying the directory of manga pages
...
autor_url = input("Введите ссылку на автора: ")

ass = " "                                                           # Начало статьи (начальный текст), при отсутствии не работает программа / The beginning of the article (the initial text), in the absence of the program does not work
files = []
...
for i in files:
    ...
    ass += a                                                        # Генерация html кода для загрузки изображений / Generating html code for uploading images
...
print(response['url'])                  # Получение ссылки на статью / Getting a link to an article
